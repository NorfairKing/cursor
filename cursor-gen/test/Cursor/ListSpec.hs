...
spec = do
  ...
  describe "emptyListCursor" $ it "is valid" $ shouldBeValid (emptyListCursor @Int)
  describe "makeListCursor" $
    it "produces valid list cursors" $ producesValidsOnValids (makeListCursor @Rational)
  describe "makeListCursorSeq" $
    it "produces valid list cursors" $ producesValidsOnValids (makeListCursorSeq @Rational)
  ...
  describe "makeListCursorWithSelectionSeq" $
    it "produces valid list cursors" $
    producesValidsOnValids2 (makeListCursorWithSelectionSeq @Rational)
  describe "rebuildListCursor" $ do
    it "produces valid lists" $ producesValidsOnValids (rebuildListCursor @Rational)
    it "is the inverse of makeListCursor" $ inverseFunctions (makeListCursor @Int) rebuildListCursor
    it "is the inverse of makeListCursorWithSelection for any index" $
      forAllUnchecked $ \i ->
        inverseFunctionsIfFirstSucceeds (makeListCursorWithSelection @Int i) rebuildListCursor
  describe "rebuildListCursorSeq" $ do
    it "produces valid lists" $ producesValidsOnValids (rebuildListCursorSeq @Rational)
    it "is the inverse of makeListCursor" $
      inverseFunctions (makeListCursorSeq @Int) rebuildListCursorSeq
    it "is the inverse of makeListCursorWithSelection for any index" $
      forAllUnchecked $ \i ->
        inverseFunctionsIfFirstSucceeds (makeListCursorWithSelectionSeq @Int i) rebuildListCursorSeq
  describe "listCursorNull" $
    it "produces valid bools" $ producesValidsOnValids (listCursorNull @Rational)
  describe "listCursorLength" $
    it "produces valid bools" $ producesValidsOnValids (listCursorLength @Rational)
  describe "listCursorIndex" $
    it "produces valid indices" $ producesValidsOnValids (listCursorIndex @Rational)
  describe "listCursorSelectPrev" $ do
    it "produces valid cursors" $ producesValidsOnValids (listCursorSelectPrev @Rational)
  ...
  describe "listCursorSelectNext" $ do
    it "produces valid cursors" $ producesValidsOnValids (listCursorSelectNext @Rational)
    it "is a movement" $ isMovementM listCursorSelectNext
    it "selects the next position" pending
  describe "listCursorSelectIndex" $ do
    it "produces valid cursors" $ producesValidsOnValids2 (listCursorSelectIndex @Rational)
  ...
  describe "listCursorPrevItem" $ do
    it "produces valid items" $ producesValidsOnValids (listCursorPrevItem @Rational)
    it "returns the item before the position" pending
  describe "listCursorNextItem" $ do
    it "produces valid items" $ producesValidsOnValids (listCursorNextItem @Rational)
    it "returns the item after the position" pending
  describe "listCursorSelectStart" $ do
    it "produces valid cursors" $ producesValidsOnValids (listCursorSelectStart @Rational)
  ...
  describe "listCursorSelectEnd" $ do
    it "produces valid cursors" $ producesValidsOnValids (listCursorSelectEnd @Rational)
  ...
  describe "listCursorInsert" $ do
    it "produces valids" $ forAllValid $ \d -> producesValidsOnValids (listCursorInsert @Rational d)
    it "inserts an item before the cursor" $ pending
  describe "listCursorAppend" $ do
    it "produces valids" $ forAllValid $ \d -> producesValidsOnValids (listCursorAppend @Rational d)
...
isMovement func =
  forAllValid $ \lec ->
    rebuildListCursor (lec :: ListCursor Int) `shouldBe` rebuildListCursor (func lec)
